        ...
        while(1) {  // main accept() loop
            ...
            if(child==0) {
                ...
                while(1){ 
                    ...
                    const char *lastSix = &response[l-6];
                    int compareForSIGINT = strncmp("SIGINT",lastSix,6);
                    if(compareForSIGINT==0){
                    ...
                    //const char *lastFive = &response[l-7];
                    //printf("lastFour=%s",lastFour);
                    int compareForQUIT = strncmp("SIGQUIT",response,7);
                    if(compareForQUIT==0){
                        printf("client said to quit");
                        waitForConnection = 1;
                        break;
